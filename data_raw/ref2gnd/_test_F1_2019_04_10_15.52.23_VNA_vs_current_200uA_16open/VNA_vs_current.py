...
## PNA for RF
PNA = stlab.adi(addr='TCPIP::192.168.1.42::INSTR')
...
prefix = '_test_F'
idstring = 'VNA_vs_current_200uA'

ivvi.RampVoltage(isdac, islist[0] / isgain * 1e3, tt=10.)
for i, curr in enumerate(islist):
    print('\n############### Iset (A): {}'.format(curr))
    ivvi.RampVoltage(
        isdac, curr / isgain * 1e3,
        tt=5.)  #Take curr in amps, apply gain and convert to millivolts
    ...
    try:
        ...
        T = -1
    data['Power (dBm)'] = float(PNA.GetPower())
    data['T (K)'] = T
    ...
    if i == 0:
        myfile = stlab.newfile(
            prefix, idstring + '_16open', data.keys(), autoindex=True)
...
caption = 'VNA vs current shorted reference'
stlab.autoplot(
    myfile,
    'Frequency (Hz)',
    'Iset (A)',
